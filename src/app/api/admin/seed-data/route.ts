...
export async function GET(req: NextRequest) {
  // Security guard — requires secret key to prevent unauthorized access
  const secret = req.nextUrl.searchParams.get("secret");
  if (secret !== "devineora-seed-2026") {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  try {
    // 1. Update Email Settings to Gmail with cleaned App Password
    const cleanPass = "fasmuowynorwlemx"; // stripped whitespace
    
    const settingsToUpdate = [
      { key: "email_provider", value: "gmail" },
      { key: "email_gmail_user", value: "devineora7@gmail.com" },
      { key: "email_gmail_pass", value: cleanPass },
      { key: "email_from_address", value: "devineora7@gmail.com" },
      { key: "email_from_name", value: "DEVINE ORA" }
    ];
    ...
    return NextResponse.json({
      success: true,
      message: "Database successfully updated with latest review dates, SEO blog articles, and Gmail SMTP configuration!",
